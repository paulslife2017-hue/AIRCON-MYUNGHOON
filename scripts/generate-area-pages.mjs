/**
 * generate-area-pages.mjs
 * areas.ts + area-page.ts 와 동일한 데이터/로직을 순수 JS로 구현하여
 * public/area/*.html 을 65개 일괄 생성합니다.
 */
...
const OUT_DIR = path.resolve(__dirname, '../public/area')
...
const repairAreas = REPAIR_REGIONS.flatMap(r =>
  REPAIR_KEYWORDS.map(k => ({
    slug: `${r.shortName}-${k.slugSuffix}`,
    name: r.name, shortName: r.shortName,
    service: 'repair', keyword: k.keyword,
    dongs: r.dongs, lat: r.lat, lng: r.lng,
  }))
)

const cleanAreas = CLEAN_REGIONS.map(r => ({
  slug: `${r.shortName}-에어컨청소`,
  name: r.name, shortName: r.shortName,
  service: 'clean', keyword: 'clean',
  dongs: r.dongs, lat: r.lat, lng: r.lng,
}))

const AREAS = [...repairAreas, ...cleanAreas]

// ─────────────────────────────────────────
// 렌더링 (area-page.ts 와 동일 로직)
// ─────────────────────────────────────────
const PHONE = '010-2343-2966'
const BASE_URL = 'https://www.airconhelper.co.kr'

const KW = {
  repair: { label: '에어컨 수리',    h1sub: '수리·냉방불량·누수 당일 출장', priceRows: [['에어컨 점검 진단','현장 안내'],['가스충전 (R410A)','7만원~'],['실내기 수리','현장 진단 후 안내'],['전기부품 교체','현장 진단 후 안내']] },
  gas:    { label: '에어컨 가스충전', h1sub: '냉매충전·누설점검 당일 출장',  priceRows: [['가스충전 (R410A)','7만원~'],['가스충전 (R22)','7만원~'],['냉매 누설 점검','현장 안내'],['충전 후 냉방 테스트','포함']] },
  check:  { label: '에어컨 점검',    h1sub: '냉방불량·이상소음 당일 점검',  priceRows: [['에어컨 기본 점검','현장 안내'],['냉매량 확인','포함'],['전기·배선 점검','포함'],['수리 필요 시','별도 안내']] },
  clean:  { label: '냉매충전',       h1sub: '냉매부족·냉방약화 즉시 해결',  priceRows: [['R410A 냉매충전','7만원~'],['R22 냉매충전','7만원~'],['냉매 누설 확인','포함'],['충전 후 테스트','포함']] },
}

const FAQ = {
  repair: [
    { q: '에어컨 수리 비용이 얼마나 되나요?', a: '에어컨 수리 비용은 고장 원인과 기종에 따라 다릅니다. 가스충전은 7만원~, 전기부품 교체는 현장 진단 후 안내드립니다. 방문 전 전화 상담으로 예상 비용을 먼저 확인하실 수 있습니다.' },
  ...
  gas: [
    ...
    { q: '가스충전 비용이 얼마나 드나요?', a: 'R410A·R22 냉매 기준 7만원~입니다. 누설 수리가 필요한 경우 추가 비용이 발생할 수 있으며, 방문 전 전화로 상담해 드립니다.' },
  ...
  clean: [
    ...
    { q: '냉매충전 비용이 얼마인가요?', a: 'R410A·R22 모두 7만원~입니다. 누설이 있는 경우 수리 비용이 추가될 수 있습니다. 방문 전 전화로 먼저 상담해 드립니다.' },
...
<div class="wrap">
  ...
  <div class="svc-grid">
    ${services.map(([ic,name,d]) => `<div class="svc-card"><i class="fas ${ic}"></i><h3>${name}</h3><p>${d}</p></div>`).join('')}
  </div>
</div>
...
for (const area of AREAS) {
  const filePath = path.join(OUT_DIR, `${area.slug}.html`)
  const html = renderAreaPage(area)
  fs.writeFileSync(filePath, html, 'utf8')
  created++
  console.log(`✅ ${area.slug}.html`)
}
